...
WORK_DIR=/home/vmplanet/dm/CUCS
...
if [ "x$clustertype" == "xkmeans" ]; then
  $MAHOUT kmeans \
    -i ${WORK_DIR}/cucs-seq-vectors-1gram/tfidf-vectors/ \
  ...
  $MAHOUT dirichlet \
    -i ${WORK_DIR}/cucs-seq-vectors-1gram/tf-vectors \
  ...
  $MAHOUT clusterdump \
    ...
    -dt sequencefile -b 100 -n 20 -sp 0 \
    && \
  cat ${WORK_DIR}/cucs-dirichlet/clusterdump
